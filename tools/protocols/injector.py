...
"""
  ...
  by Marc Newlin (@macrnewlin) and
  Matthias Deeg (@matthiasdeeg, matthias.deeg@syss.de)

  Copyright (c) 2019 Marc Newlin
  Copyright (c) 2016,2019 SySS GmbH
...
class Injector(object):
    ...
    def __init__(self, protocol, keymap):
        """Constructor"""
    ...
    def get_modifiers(self, shift=False, ctrl=False, alt_l=False, alt_r=False, win=False):
        """Get modifiers"""
        ...
        if ctrl:
            modifiers |= MODIFIER_CONTROL_LEFT
        if alt_l:
            modifiers |= MODIFIER_ALT_LEFT
        if alt_r:
            modifiers |= MODIFIER_ALT_RIGHT
        if win:
            modifiers |= MODIFIER_GUI_LEFT

        return modifiers

    def send_enter(self, shift=False, ctrl=False, alt_l=False, alt_r=False, win=False):
        """Send ENTER key"""

        modifiers = self.get_modifiers(shift, ctrl, alt_l, alt_r, win)
        self.protocol.send_hid_event(scan_code=KEY_RETURN, modifiers=modifiers)
        self.protocol.send_hid_event(scan_code=0x00, modifiers=modifiers)

    def send_escape(self, shift=False, ctrl=False, alt_l=False, alt_r=False, win=False):
        """Send ESCAPE key"""

        modifiers = self.get_modifiers(shift, ctrl, alt_l, alt_r, win)
        self.protocol.send_hid_event(scan_code=KEY_ESCAPE, modifiers=modifiers)
        self.protocol.send_hid_event(scan_code=0x00)

    def send_backspace(self, shift=False, ctrl=False, alt_l=False, alt_r=False, win=False):
        """Send BACKSPACE key"""

        modifiers = self.get_modifiers(shift, ctrl, alt_l, alt_r, win)
        self.protocol.send_hid_event(scan_code=KEY_BACKSPACE, modifiers=modifiers)
        self.protocol.send_hid_event(scan_code=0x00)

    def send_tab(self, shift=False, ctrl=False, alt_l=False, alt_r=False, win=False):
        """Send TAB key"""

        modifiers = self.get_modifiers(shift, ctrl, alt_l, alt_r, win)
        self.protocol.send_hid_event(scan_code=KEY_TAB, modifiers=modifiers)
        self.protocol.send_hid_event(scan_code=0x00)

    def send_capslock(self, shift=False, ctrl=False, alt_l=False, alt_r=False, win=False):
        """Send CAPS LOCK key"""

        modifiers = self.get_modifiers(shift, ctrl, alt_l, alt_r, win)
        self.protocol.send_hid_event(scan_code=KEY_CAPS_LOCK, modifiers=modifiers)
        self.protocol.send_hid_event(scan_code=0x00)

    def send_key(self, key, shift=False, ctrl=False, alt_l=False, alt_r=False, win=False):
        """Send key"""

        modifiers = self.get_modifiers(shift, ctrl, alt_l, alt_r, win)
        self.protocol.send_hid_event(scan_code=key, modifiers=modifiers)
        self.protocol.send_hid_event(scan_code=0x00)
    ...
    def send_string(self, string):
        ...
        for c in string:
            # Get modifier and scan code from current keymap
            modifiers, scan_code = self.keymap[c]

            # Send keypress
            self.protocol.send_hid_event(scan_code, modifiers)

            # Send key release
            self.protocol.send_hid_event(scan_code=0x00, modifiers=MODIFIER_NONE)
