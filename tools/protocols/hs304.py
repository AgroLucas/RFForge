#!/usr/bin/env/python3
# -*- coding: utf-8 -*-

import crcmod
import logging
import struct
import time

from .protocol import Protocol
from lib import common

TRANSMIT_DELAY = 0.002

class HS304(Protocol):
    """ HS304 keyboard/mouse transceiver"""

    # HS304 devices analyzed by Marc Newlin
    flavor1 = {"magic": b"\x31\x78",
               "crc8": True,
               "crc8_init":  0x1d}

    # BEBONCOOL wireless presenter analyzed by Matthias Deeg
    # there is a CRC8, but its value doesn't matter
    flavor2 = {"magic": b"\x94\x54",
               "crc8": True,
               "crc8_init":  0x59}

    # Red Star Tec wireless presenter analyzed by Matthias Deeg
    # there is a static value which doesn't matter
    flavor3 = {"magic": b"\xD2\x43",
               "crc8": False,
               "crc8_init":  0x88}

    # flavors of analyzed HS304 devices
    flavors = {"hs304_common": flavor1,
               "hs304_beboncool_eu": flavor2,
               "hs304_red_star_tec_eu": flavor3}

    # Lookup table for payload byte 0 (Keyboard HID byte 2)
    LUT0 = [[].index(x) for x in range(256)]

    # Lookup table for payload byte 3 (Mouse HID byte 0)
    LUT3 = [[].index(x) for x in range(256)]

    # Lookup table for payload byte 4 (Mouse HID byte 1)
    LUT4 = [[].index(x) for x in range(256)]

    # Lookup table for payload byte 5 (Mouse HID byte 2)
    LUT5 = [[].index(x) for x in range(256)]

    # Lookup table for payload byte 6 (Keyboard HID byte 1)
    LUT6 = [[].index(x) for x in range(256)]

    def __init__(self, flavor="hs304_common"):
        """Constructor"""

        # initialize protocol
        super(HS304, self).__init__("HS304")

        # get configuration depending on flavor
        self.config = HS304.flavors[flavor]

        # Initialize CRC generators
        self.CRC16 = crcmod.mkCrcFun(0x11021, initCrc=0x422e, rev=False, xorOut=0x0000)
        self.CRC8 = crcmod.mkCrcFun(0x101, initCrc=self.config["crc8_init"], rev=False, xorOut=0x00)

    def configure_radio(self):
        """Configure the radio"""

        # Put the radio in promiscuous mode with static 4 byte address
        common.radio.enter_promiscuous_mode_generic(b"\x44\x75\x94\xE1", rate=common.RF_RATE_1M)

        # Set the channel to 7
        common.channels = [7]

        # Set the initial channel
        common.radio.set_channel(common.channels[0])

    def send_hid_event(self, scan_code=0, modifiers=0):
        """Send a HID keystroke packet"""

        # Sync word
        sync = b"\x44\x75\x94\xE1"

        # Mouse X/Y and button flags
        mouse_y = 0
        mouse_x = 0
        mouse_b = 0

        # Create payload
        payload = struct.pack("7B",
                              HS304.LUT0[scan_code],        # keyboard scan code
                              self.config["magic"][0],      # magic byte 1
                              self.config["magic"][1],      # magic byte 2
                              HS304.LUT3[mouse_b],          # mouse buttons
                              HS304.LUT4[mouse_x],          # mouse x
                              HS304.LUT5[mouse_y],          # mouse y
                              HS304.LUT6[modifiers])        # modifiers

        # set last HS304 payload byte
        p = sync + payload
        if self.config["crc8"]:
            # Append CRC-8
            p += struct.pack("B", self.CRC8(p))
        else:
            # Or further static magic value
            p += struct.pack("B", self.config["crc8_init"])

        # Append CRC-16
        p += struct.pack("!H", self.CRC16(p))

        # Transmit the packet 10 times on channel 7 (2.407 GHz)
        for x in range(10):
            payload = b"\xF1\x0F\x55" + p + b"\xAF\xFF"
            common.radio.transmit_payload_generic(address=b"\x00\x00\x00\x00\x00",
                                                  payload=b"\xF1\x0F\x55" + p + b"\xAF\xFF")
            time.sleep(TRANSMIT_DELAY)

    def discovery_loop(self, cancel):
        """Discovery loop"""

        while not cancel:
            # Receive payloads
            value = common.radio.receive_payload()

            if value[0] == 0xFF:
                continue

            if len(value) < 15:
                continue

            # Verify the CRC
            crc_calc = self.CRC16(value[0:12])
            crc = struct.unpack("!H", value[12:14])[0]
            if crc != crc_calc:
                logging.debug("CRC failure")
                continue

            # Parse the payload
            sync = value[0:4]
            payload = value[4:]
            code = self.LUT0.index(payload[0])
            magic1 = payload[1]
            magic2 = payload[2]
            mouse_b = self.LUT3.index(payload[3])
            mouse_x = self.LUT4.index(payload[4])
            mouse_y = self.LUT5.index(payload[5])
            modifiers = self.LUT6.index(payload[6])

            # Log the packet
            logging.info("Scan Code: {:02x}, Magic 1: {:02x}, Magic 2: {:02x} "
                         "Mouse Button: {:02x}, Mouse X: {}, Mouse Y: {}, "
                         "Modifier: {:02x}"
                         .format(code, magic1, magic2, mouse_b, mouse_x,
                                 mouse_y, modifiers))

    def start_injection(self):
        """Enter injection mode"""
        return

    def stop_injection(self):
        """Leave injection mode"""
        return
